    ...
    <div class="card">
        <div class="card-header">
            <ul class="navbar list-unstyled m-0 p-0">
                <li>
                    <h3 class="card-title">{{ __('lang.rooms') }}</h3>
                </li>
                <li>

                    <!-- SEARCH FORM -->
                    <form action="{{ route('rooms.index') }}" method="get" class="form-inline m-0 ml-md-3">
                        @csrf
        ...
        <div class="card-body pt-0 table-responsive">
            <table id="example1" class="table table-bordered table-striped ">
                ...
                <tr>
                    ...
                    <td class="py-1">{{ $room->group_names }}</td>
                    <td class="py-1">{{ $room->students }}</td>
                    <td class="py-1">
                        ...
                        @if( Auth::user()->role == 1 || Auth::user()->role == 2)
                                        ...
                                        <form action="{{ route('rooms.update', $room->id) }}" method="post" enctype="multipart/form-data">
                                            ...
                                            <div class="modal-body">
                                                ...
                                                <div class="card-body">
                                                    <div class="form-group">
                                                        <label for="exampleInputEmail1">{{ __('lang.name') }}</label>
                                                        <input name="name" type="text" class="form-control" value="{{ $room->name }}" id="exampleInputEmail1" placeholder="{{ __('lang.type_name') }}">
                                                    </div>
                            ...
                            <button type="button" class="btn btn-danger my-0 py-0 px-1" data-toggle="modal" data-target="#modal-danger{{ $room->id }}">
                                <i class="fas fa-trash m-0 p-0"></i>
                            </button>

                            <div class="modal fade" id="modal-danger{{ $room->id }}">
                                <div class="modal-dialog">
                                    <div class="modal-content bg-danger">
                                        ...
                                        <div class="modal-footer justify-content-between">
                                            <button type="button" class="btn btn-outline-light" data-dismiss="modal">{{ __('lang.close') }}</button>
                                                <form class="d-inline" action="{{ route('rooms.destroy',$room->id) }}" method="post">
                                                    @csrf
                                                    @method('DELETE')
            ...
            <div>{{ $rooms->appends($_GET)->links() }}</div>
        </div>
