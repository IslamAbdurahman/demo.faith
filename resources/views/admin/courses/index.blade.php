    ...
    <div class="card">
        <div class="card-header">
            <ul class="navbar list-unstyled m-0 p-0">
                <li>
                    <h3 class="card-title">{{ __('lang.courses') }}</h3>
                </li>
                <li>

                    <!-- SEARCH FORM -->
                    <form action="{{ route('courses.index') }}" method="get" class="form-inline m-0 ml-md-3">
                        @csrf
        ...
        <div class="card-body pt-0 table-responsive">
            <table id="example1" class="table table-bordered table-striped ">
                ...
                <tr>
                    <td class="py-1">
                        {{($courses->currentpage()-1)*$courses->perpage()+($loop->index+1)}}
                    </td>
                    <td class="py-1">{{ $course->name }}</td>
                    <td class="py-1">{{ $course->count_groups }}</td>
                    <td class="py-1">{{ $course->students }}</td>
                    <td class="py-1">
                        ...
                        @if( Auth::user()->role == 1  || Auth::user()->role == 2)
                                        ...
                                        <form action="{{ route('courses.update', $course->id) }}" method="post" enctype="multipart/form-data">
                                            ...
                                            <div class="modal-body">
                                                ...
                                                <div class="card-body">
                                                    <div class="form-group">
                                                        <label for="exampleInputEmail1">{{ __('lang.name') }}</label>
                                                        <input name="name" type="text" class="form-control" value="{{ $course->name }}" id="exampleInputEmail1" placeholder="{{ __('lang.type_name') }}">
                                                    </div>
                            ...
                            <button type="button" class="btn btn-danger my-0 py-0 px-1" data-toggle="modal" data-target="#modal-danger{{ $course->id }}">
                                <i class="fas fa-trash m-0 p-0"></i>
                            </button>

                            <div class="modal fade" id="modal-danger{{ $course->id }}">
                                <div class="modal-dialog">
            ...
            <div>{{ $courses->appends($_GET)->links() }}</div>
        </div>
        <!-- /.card-body -->
@endsection
