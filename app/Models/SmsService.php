...
class SmsService extends Model
{
    ...
    {
        $service = SmsService::where('status', '=', 1)->first();
        if ($service && $service->name == 'eskiz') {
            $sms = SmsService::eskiz($num, $message);
        } elseif ($service && $service->name == 'sysdc') {
            $sms = SmsService::sysdc($num, $message);
        } elseif ($service && $service->name == 'play_mobile') {
            $sms = SmsService::play_mobile($num, $message);
        } else {
            $sms = 0;
        }
    ...
    public static function eskiz($num, $message)
    {
        try {
            ...
            $diff = Carbon::parse($service->updated_at)->diff(Carbon::parse(tash_time()))->days;
            ...
            if ($diff > 29) {
                ...
                $eskiz = SmsService::firstOrCreate(['name' => 'eskiz']);
    ...
    public static function sysdc($num, $message)
    {
        try {
            ...
            $service = SmsService::where('name', '=', 'sysdc')->first();
    ...
    public static function play_mobile($num, $message)
    {
        ...
        try {
            $service = SmsService::where('name', '=', 'play_mobile')->first();
